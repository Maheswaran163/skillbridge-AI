...
export interface SkillMaster {
  ...
  category: 'technical' | 'soft' | 'domain';
  description: string;
  popularDemandScore: number; // 0 - 100
}
...
export interface JobOpportunity {
  ...
  deadline: string;
  createdAt: string;
  institutionId?: string; // Multi-tenant scoping if targeted
}
...
export interface CandidateMatchResult {
  ...
  weightedMatchScore: number; // 0 - 100
  scoreBreakdown: {
    skillMatchScore: number; // 50%
    assessmentScore: number; // 15%
    projectsScore: number; // 10%
    certificationsScore: number; // 5%
    experienceScore: number; // 5%
    careerInterestScore: number; // 10%
    softSkillsScore: number; // 5%
  };
